...
DETAILS_TEXT = (
    "ЧТО ТАКОЕ ПРОЕКТ «КАК НАЙТИ СВОЁ ПРЕДНАЗНАЧЕНИЕ»?\n\n"
    "Я создала этот курс для тех, кто потерял себя, и для тех, кто до сих пор не знает, кем хочет быть, когда вырастет🥺\n\n"
    "Если вы:\n"
    "- задаётесь вопросом о смысле жизни [своей жизни]\n"
    "- не знаете, чем хотели бы заниматься и что сделает вас счастливыми\n"
    "- не хотите прожить всю жизнь в вечных поисках,\n"
    "Я хочу вам помочь!🫂\n\n"
    "На проекте вы не просто поймёте, какая профессия вам подходит. Это будет ГЛУБИННАЯ РАБОТА, благодаря которой вы:\n"
    "• научитесь понимать и слышать себя\n"
    "• выстраивать более здоровые отношения с собой и окружающими\n"
    "• сможете хорошо узнать себя и познакомиться со своими убеждениями, ценностями, мечтами\n"
    "• вы научитесь планировать свою жизнь так, чтобы каждое действие вело вас к исполнению мечты и реализации предназначения\n\n"
    "📌Курс будет в формате закрытого телеграм-канала: каждый день вам будет приходить различные задания и практики, на которые нужно будет выделять от 5 до 60 минут в день.\n\n"
    "Также у нас будет общий чат со всеми участниками, где мы будем делиться своими достижениями и трудностями, поддерживать друг друга. "
    "Я тоже всегда буду на связи, чтобы помочь или ответить на вопрос!🤲🏻\n\n"
    "Никаких скучных лекций и нравоучений, только практика, которая поможет вам по-настоящему найти себя❤️‍🔥\n\n"
    "На фото ты можешь изучить подробное наполнение 24 дней курса, а волшебная кнопочка ниже перенаправит тебя на путь исследования своей миссии👇🏻"
)
...
def build_schedule(start_dt: datetime) -> List[Tuple[int, int, int]]:
    ...
    schedule.append((99, dt_to_ts(discount_until), dt_to_ts(discount_until)))  # "скидка кончилась"
...
async def send_step(user_id: int, step: int):
    ...
    if step == 2:
        ...
        await bot.send_message(user_id, "⏳ Час прошёл. Скидка больше не действует.")
    elif step == 3:
